...
export async function atualizarPublico(proposta_id: string, formData: FormData) {
  ...
  if (presId)
    formacaoUpdates.push(supabase.from('proposta_servicos')
      .update({ quantidade: (horasPresencial && horasPresencial !== '') ? Number(horasPresencial) : 8 }).eq('id', presId))
  if (eadId)
    formacaoUpdates.push(supabase.from('proposta_servicos')
      .update({ quantidade: (horasEAD && horasEAD !== '') ? Number(horasEAD) : 10 }).eq('id', eadId))
  if (assId)
    formacaoUpdates.push(supabase.from('proposta_servicos')
      .update({ quantidade: (horasAssessoria && horasAssessoria !== '') ? Number(horasAssessoria) : 10 }).eq('id', assId))
