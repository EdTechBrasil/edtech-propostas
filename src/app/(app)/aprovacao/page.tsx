      ...
      {pendentes.length === 0 ? (
        ...
        <div className="space-y-4">
          {pendentes.map((a: any) => {
            ...
            return (
              <Card key={a.id} className="border-yellow-200 bg-yellow-50/30">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs text-slate-500 font-mono">{proposta?.id?.slice(0, 8)}…</span>
                      </div>

                      {/* Info da proposta */}
                      <p className="font-semibold text-slate-900 truncate">
                        {proposta?.cliente_nome_instituicao || <span className="text-slate-400 font-normal italic">Sem dados do cliente</span>}
                      </p>
                      <p className="text-sm text-slate-500 mt-0.5">
                        Solicitado por <strong className="text-slate-700">{a.solicitante?.nome ?? '—'}</strong>
                        {' · '}Orçamento: <strong className="text-slate-700">{formatCurrency(proposta?.orcamento_alvo ?? 0)}</strong>
                      </p>

                      {/* Indicador de margem */}
                      <div className="flex items-center gap-4 mt-3">
                        <div className="flex items-center gap-2 rounded-lg bg-red-100 border border-red-200 px-3 py-1.5">
